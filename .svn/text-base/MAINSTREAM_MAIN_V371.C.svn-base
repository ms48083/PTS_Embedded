/****************************************************************
   ...
   Created:    Jun 12, 2006 by Mike Schwedt (C) MS Technology Solutions LLC
   ...
   15-Sep-08   v369 Change LCD_GetNumberEntry so that <OK> with no entry is like cancel
   26-May-09   v370 Add date range to log download
                    Change LCD_GetNumberEntry so caller decides if <OK> with no entry is like cancel
   30-Jun-09   v371 Improve logging of purge events by saving the purged-to stations
                    Dest_sta contains the first purged-to station and Flags contain bit mask of all purged-to stations

*****************************************************************/
#define FIRMWARE_VERSION "Mainstream V3.71"
...
{  // Define all parameters that are saved in flash
   ...
   //char point2point;      // if true, single micro for a point to point configuration
} param;
...
void syncParameters(void);  // to send parameter settings to the remotes
...
MYDEBUG void refresh_timer_message(char msg);
...
void lcd_showTransPage(long page, long lastPage);
...
char priority, system_direction, memory_state;
char btnStatFlag;
char arrival_from;
...
{  // Setup system i/o processing routine
   ...
   arrival_alert=0;
   for (i=0; i<10; i++) { autoReturnTime[i]=0; }   // initialize auto-return timers
   main_arrival=0;
   arrival_from=0;
   systemStation=0;
   systemStationb=0;
...
// P2P WORK TO BE DONE HERE
      ...
      if (response.command==INPUTS_ARE)
      {
        ...
        if (arrival_alert)
        {
           arrival_alert &= (remote_data[REMOTE_CIC]
                         ...
                         & ~systemStationb;
           // for each zero bit clear the auto-return timer
           for (k=1; k<=LAST_STATION; k++)
           {  if ((arrival_alert & 1<<(k-1))==0) autoReturnTime[k]=0;
           }
        }
   ...
   switch (system_state)
   {
      case IDLE_STATE:
         ...
         else if (!priority && (remote_data[REMOTE_RTS2] & STATION_SET))
         ...
         {  // check main request
            ...
            {  systemStation=bit2station(systemStationb);
               ...
               if (lcd_autoReturn()) translog.flags = FLAG_ARETURN;
            }
         }
      ...
      case WAIT_FOR_DIVERTERS:   // does job for send and return
         ...
         // Ready?
	      ...
	      {  // timeout
            ...
            {   // blower timeout
	            statistics.divert_alarm++;
	            translog.status=STS_BLOWER_TOUT; translog.end_tm=SEC_TIMER;
         ...
         {  // Cancel the transaction
	         translog.status=STS_CANCEL_BY_SEND; translog.end_tm=SEC_TIMER;
...
// P2P WORK TO BE DONE HERE
               ...
               if (response.data[0] == availableDevices)  // all stations ok?
               {
                  ...
                  {  if (carrierInChamber(mainStation))
                     ...
                     {  print_event("TRANSACTION CANCELED - CARRIER REMOVED");
				            translog.status=STS_CANCEL_BY_CIC; translog.end_tm=SEC_TIMER;
                        //print_transaction( translog );
				            addTransaction( translog );
                        new_state=CANCEL_STATE;
                     }
                  ...
                  {  if (remote_data[REMOTE_CIC] & systemStationb)
                     ...
                     {  print_event("TRANSACTION CANCELED - CARRIER REMOVED");
				            translog.status=STS_CANCEL_BY_REMOTE; translog.end_tm=SEC_TIMER;
                        //print_transaction( translog );
				            addTransaction( translog );
                        new_state=CANCEL_STATE;
                     }
         ...
         {  new_state=CANCEL_STATE;  // no stacking allowed
	         translog.status=STS_CANCEL_BY_STACK; translog.end_tm=SEC_TIMER;
	         //print_transaction( translog );
	         addTransaction( translog );
         }
         break;
...
void processMainArrivalAlert(void)
{
	...
	if (main_arrival)
	{
      ...
      if (main_arrival==1)                        		 do_alert(ON);  // constant on
	   else if ( (SEC_TIMER - main_arrival) %20 >= 10 ) do_alert(ON);  // pulsed on
	   else                                        		 do_alert(OFF);
...
// P2P WORK TO BE DONE HERE
   ...
   if ((diverter_map[station]==0) || (di_diverterPos==diverter_map[station])) diverter_status |= 0x100;
...
const struct{
   ...
   char max;       // parameter maximum (or don't save flag changes [=0])
 ...
 MNU_CHECK_SFLASH,   MNU_NONE, "CHECK SERIAL FLASH", MNU_OTHR, &NOch, 0, 0, "",
 MNU_ANALYZE_LOG,    MNU_NONE, "ANALYZE EVENT LOG",  MNU_OTHR, &NOch, 0, 0, "",
 MNU_SUBSTA_ADDR,    MNU_ADV,  "SUB-STA ADDRESSING", MNU_FLAG, &param.subStaAddressing, 0, 1, "",
 MNU_BLOWER_TYPE,    MNU_ADV,  "SET BLOWER TYPE",    MNU_OTHR, &param.blowerType, 0, 0, "",
 MNU_PORT_MAPPING,   MNU_ADV,  "SET PORT MAPPING",   MNU_OTHR, &NOch, 0, 0, "",
...
void purgeSystem(char auto_purge)
{
   ...
   //eventlog.source_sta // Not available for use
...
{  // returns 0 if no error, non-zero otherwise
   ...
   struct iomessage command;
	...
	// send command to remote to update and save parameters
...
// }
   ...
   idx=0;
   buf[idx++] = 6; // system id
   buf[idx++] = 2;  // packet type = heartbeat
   buf[idx++] = *cptr++; // (char) SEC_TIMER; // Seconds timer
   buf[idx++] = *cptr++; //(char) SEC_TIMER; // Seconds timer
   buf[idx++] = *cptr++; //(char) SEC_TIMER; // Seconds timer
   buf[idx++] = *cptr++; //(char) SEC_TIMER; // Seconds timer
   buf[idx++] = system_state;
   buf[idx++] = systemStation;
   buf[idx++] = mainStation;
   buf[idx++] = system_direction;
   buf[idx++] = main2main_trans;
   buf[idx++] = destMain;
   buf[idx++] = remote_data[REMOTE_CIC];
   buf[idx++] = remote_data[REMOTE_DOOR];
   buf[idx++] = remote_data[REMOTE_ARRIVE];
   buf[idx++] = remote_data[REMOTE_RTS];
   buf[idx++] = remote_data[REMOTE_RTS2];
   buf[idx++] = 0; // local inputs
   buf[idx++] = 0; // slave inputs

   length = idx; // length = strlen(buf) + 1;
         ...
         while (!good && retry)
         {
            ...
            {  // sent command ok, now get response
               ...
               {  // OR all the input responses together
                  ...
                  if ((workmsg.devAddr==7) && (workmsg.command==RETURN_INPUTS))
...
void slaveProcessCommand(struct iomessage message)
{
   ...
   case RETURN_INPUTS:        /* return general status */
      ...
      if (lcd_autoReturn()) slaveReturnStatus |= 0x80;  // auto return requested
...
{	// page 0 shows the last transaction
   ...
   while ( (j < TRANS_PER_PAGE) && (entry >= 0) ) //< sizeOfTransLog()))
   {
      ...
      {  mktm(&time, trans.start_tm);
         ...
         // Display record unless it is blank
