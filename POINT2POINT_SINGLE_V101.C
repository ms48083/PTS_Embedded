/****************************************************************
   ...
   11-Dec-19   v101 fix CIC output assignments, reverse Pause input logic, add purge back into the menu
                    Setup pause as a momentary toggle with debounce

*****************************************************************/
#define FIRMWARE_VERSION "Point2Point V1.01"
#define VERS                            1
#define SUBVERS                         01
...
#define do_doorLock(value)		  setDigOutput(1,value)
#define do_inUse(value)         setDigOutput(0,value)
#define do_priorityLight(value)    setDigOutput(99+do_shift,value)
#define do_CICLight(value)         setDigOutput(2,value)
...
#define do_CICLight2(value)        setDigOutput(3,value)
...
{  lastComTimer = MS_TIMER;
   ...
   //{
      ...
      //{
        ...
        if (param.point2point)
        {  // flash inuse if either CIC or either door
        		if (di_carrierInChamber || di_carrierInChamber2 || !di_doorClosed || !di_doorClosed2)
            {	inUse(FLASH, 1 & ~systemStationb);
           	} else
            {  inUse(OFF, 1 & ~systemStationb);
            }
        } else
        {
	        // flash inuse when cic and not stacking
...
// P2P WORK TO BE DONE HERE
   ...
   if (new_state != system_state)
   {
      ...
      if (new_state==IDLE_STATE) do_doorLock(OFF);
      else do_doorLock(ON);
   }
...
char paused()
{  // processes di_pause inputs as debounced toggles of the paused state
   // returns the current status of the toggle, paused (TRUE) or not (FALSE)
	static char isPaused;
   static char isCaptured;
   static unsigned long pushTimer;
   ...
   // Is pause being pressed and we haven't captured it yet
   if ((di_pause || di_pause2) && (isCaptured == FALSE))
   {  // button is pushed
   	if (pushTimer == 0)
      {  pushTimer = MS_TIMER;
      } else if (((MS_TIMER - pushTimer) > 100) && (isCaptured == FALSE))  // debounce timer in msec and not yet acknowledged
      {  // accept it and toggle the state
      	isPaused = !isPaused;
      	isCaptured = TRUE;
      }
   } else
   {  // no more push of buttons
   	pushTimer = 0;
   	isCaptured = FALSE;
   }
...
// P2P WORK TO BE DONE HERE
   ...
   // ALSO CHECK PAUSE
   if ((param.pauseEnabled) && paused()) // (di_pause || di_pause2))
   {  // enter pause mode
      memory_state=calling_state;
      setAlerts(ON);
      alarm(ON);
...
void setupMenuOrder(char menuLevel)
{
   ...
   case 1: // Basic User
      ...
      //if (param.point2point==FALSE)
      //{  // no purge -> change v101 15-Dec-19 per Joe, keep purge in menu
...
{  // handle all regularly scheduled calls
   ...
   // process flashing inUse lights
	if (param.point2point)
	{	if ((MS_TIMER %500) < 300) do_inUse( ((outputvalue[devInUseFlash] | outputvalue[devInUseSolid])) & 0x01);
   	else do_inUse( (outputvalue[devInUseSolid]) & 0x01);
   } else if (RN1100_Found)
