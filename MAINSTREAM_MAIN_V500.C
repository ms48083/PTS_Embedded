...
*****************************************************************/
#define FIRMWARE_VERSION "Mainstream V5.00"
...
void slaveSendResponse(struct RemoteMessageType message);
...
{  // display watchdog, powerlow resets and versions on the lcd
   ...
   // now send everyone a device info request
   ...
   for (i=0; i<MAX_DEVADDRS; i++)
   {  if (deviceInfo[i].active)
      {  sprintf(msgbuf, "Dev %d ver %d.%02d %2X%2X%2X %-15.15s\n", i, deviceInfo[i].version, deviceInfo[i].subversion,
...
{  // send the message periodically until all devices have acknowledged
   ...
   sendAgain=MS_TIMER;
   sendRemoteCommand(message, DEV_BROADCAST);
   TOStart=MS_TIMER;
...
// Sends secure card parameters the remote stations
   ...
   *(unsigned long*)&HBmessage.data[5] = param.cardR9Max; //
   sendRemoteCommand(HBmessage, DEV_BROADCAST); // ignore return value, will send again periodically
}
...
void processRemoteMessage(struct RemoteMessageType UDPmessage)
{
   ...
   if ((SEC_TIMER - lastComCheck) == 2)
   {
      if (recentDevices != availableDevices)
      {  comError = (availableDevices &~ recentDevices); }
      else { comError = 0; }
      lastComCheck=SEC_TIMER;
      recentDevices=0;
   } else if ((SEC_TIMER - lastComCheck) != 0)
...
/********************************************************/
void slaveSendResponse(struct RemoteMessageType UDPmessage)
{
	// Send the supplied command over UDP
   // Appends some info to the standard message
   //   .device
...
/* Repeatedly call this function to process incoming commands efficiently. */
   ...
   length = sizeof(buf);
   retval = udp_recv(&remoteSocket, buf, length);
   if (retval < -1) {
...
{  // for master to send the date/time to the slave
   ...
   sendRemoteCommand(command, DEV_BROADCAST);
}
void syncTransCount()
...
{  // let slave use the comm for a while
   ...
   *(int*)&mymsg.data[1]=availableDevices;  // DON'T SEEM TO USE IT
   ...
   sendRemoteCommand(mymsg, DEV_BROADCAST);
...
//      availableDevices=*(int *)&message.data[1] & 0x7F; // exclude slave (me)
//      FIRST_DEVADDR=0;
//      LAST_DEVADDR=0;
//      for (i=0; i<7; i++)
...
{  // Show a device record on the screen
	char buf[80];
   char ipbuf[16];
   sprintf(buf, "%2X%2X%2X %15s %3d %1d %3d %4d\n",
      device.MAC[3], device.MAC[4], device.MAC[5],
      inet_ntoa(ipbuf,device.ipAddr),
