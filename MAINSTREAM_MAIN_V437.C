/****************************************************************
   ...
   22-Sep-13   v420 Enable use of TCP/IP drivers, add Visual Alert option and take over output 15 from CIC light
                    Changed alert menu text, Added option to disable tube dryer mode, make MainAudibleAlert sticky
   12-Dec-13   v421 Force remote arrival alert on if transaction is set for auto-return (like stat does)
   				     Include first TCP server request
   20-Dec-13   v422 Add individual user identification
   					  NOT YET RELEASED
   06-Jun-14   v423 Log every card scan
   15-Aug-14   v424 Improve com when sending UID's; Set limit to 1100 users
   15-Dec-14   v425 Revert back to 1000 users to test
   16-Dec-14   v426 Default server IP address if not specified or invalid
   ...
   26-Apr-15   v430 Add sock_close() after receiving tcp server data
   27-Sep-15   v431 Support to capture standard card scans
                    Reduce TCP timeout to 13 seconds
                    Implement retry on TCP server check
   05-Oct-15   v432 Don't leave UDP socket open
   03-Jan-16   v433 Add support to capture unauthorized card scans
   22-May-16   v434 Add option to select card format 26 or 40 bits
   28-Jan-17   v435 Fix UID_Encode to support small card numbers, <9 digits
   25-Feb-17   v436 Enable operation without pushbuttons, all user I/O via touchscreen (lcd_defineActiveStations)
   27-Aug-17   v437 Touchscreen remote using slave capability
   					  Update to full ethernet
                    Simple point-to-point interface

*****************************************************************/
#define FIRMWARE_VERSION "Mainstream V4.37"
#define VERS                            4
#define SUBVERS                         37
...
//   char subversion;       // Main subversion
   ...
   char reserved[96];    // to separate standard parameters from the user id's
...
#define di_requestToSend     		(RN1100_Found ? ((~digBankIn(1)) & 0xFF) : 0)
...
/////
#define do_blowerVac(value)        RN1100_Found ? setDigOutput(12+do_shift,value) : setDigOutput(4+do_shift,value)
#define do_blowerPrs(value)        RN1100_Found ? setDigOutput(13+do_shift,value) : setDigOutput(5+do_shift,value)
#define do_blower(value)           RN1100_Found ? setDigOutput(9+do_shift,value)  : setDigOutput(1+do_shift,value)
#define do_blowerDly(value)		  RN1100_Found ? setDigOutput(16+do_shift,value) : setDigOutput(6+do_shift,value)
#define do_audibleAlert(value)     RN1100_Found ? setDigOutput(11+do_shift,value) : setDigOutput(3+do_shift,value)
#define do_diverter(value)         RN1100_Found ? setDigOutput(8+do_shift,value)  : setDigOutput(0+do_shift,value)
#define do_alarmSound(value)       RN1100_Found ? setDigOutput(10+do_shift,value) : setDigOutput(2+do_shift,value)
////
...
//char mybuf[13];
   ...
   testtime = MS_TIMER;
   while (!ifstatus(IF_ETH0) && ((MS_TIMER - testtime) < 30000)) {tcp_tick(NULL); msDelay(100); printf("-"); }
   if (!ifstatus(IF_ETH0))
...
//   }
   ...
   // check if a point to point FORCE IT FOR NOW
...
int sendRS485toUDP(struct iomessage RSmsg)
{
   ...
   // but only command, station, and data
...
//      TLmessage.data[sizeof(translog)] = param.cardL3Digits-CARDHBOFFSET; // add on 5th byte of 38 bit card id
   ...
   sendUDPcommand(TLmessage, UDP_HOST); // ignore return value, will send again periodically
}
...
{  // determine which remotes are connected to the bus
   ...
   slaveAvailable = FALSE;  // ASSUME NOT
   lcd_ClearScreen();
	lcd_DispText("Detecting Devices\n",0, 0, MODE_NORMAL);
   // hold for a few seconds
   //msDelay(100);
   ...
   while (MS_TIMER <= checkTill)
   {
      ...
      {  processUDPcommand(UDPmessage);  // process it as normal first
         ...
         {  // include the device in the deviceSet
            ...
            if (deviceSet != lastDeviceSet)
            {	sprintf(mymsg, "Dev %d; Sta %d; Ver %d.%d\n", UDPmessage.device, UDPmessage.station,
...
{  // let slave use the comm for a while
   ...
   // display lcd message
   lcd_printMessage(2, " REMOTE IN USE  ");
...
{  // for slave to handle local I/O processing
   ...
   if (getUDPcommand(&UDPmessage) > 0) slaveProcessUDPcommand(UDPmessage);
   ...
   case 1:
	...
	//       lcd_Font("13B");
   		...
   		else // point to point stuff
	      {
	         ...
	         // Draw a box for menu/help
	         lcd_BFcolorsB( lcd_LGREEN, lcd_LGREEN );   // inside color
	         lcd_Rectangle(5, 197, 315, 237, 1);      // inside paint
	         lcd_BFcolorsB( lcd_BLUE, lcd_WHITE);     // border color
	         lcd_Rectangle(5, 197, 315, 237, 0);      // border paint
