/****************************************************************
   MAINSTREAM_MAIN_V5xx.C

   Purpose:    Main station master control program for Pneumatic Tube Control System
               Works with MAINSTREAM_REMOTE_V5xx and MAINSTREAM_DIVERTER_V5xx.

   Target:     Z-World BL2500 Rabbit

   Language:   Dynamic C 9.x
...
*****************************************************************/
#define FIRMWARE_VERSION "Mainstream V5.01"
#define mVERSION                       5
#define mSUBVERSION                    01
...
#define NUMUDPDATA 288
struct ServerMessageType  // UDP based messages
{
   char system;
	char device;   // system number
...
#define NUMRMTDATA 20
struct RemoteMessageType  // UDP based messages
{
   char system;
	char device;
   char devType;  // M for main station
   ...
   char sequence; // rolling packet sequence number
   unsigned long timestamp;  // MS_TIMER
   char data[NUMRMTDATA];
};
struct PayloadType   // data package sent to remotes
{  // header elements must be the same as RemoteMessageType
   char system;
	char device;
   char devType;  // M for main station
   char command;  // can be heartbeat, event, transaction
   ...
   unsigned long timestamp;  // MS_TIMER
	char mainArrival;
   char systemState;
   char remoteAlert;  // used to send special alert/alarm behaviors
   char transStation;
   char mainStation;
   ...
   char mainBusy;
   char headDivAlign;
   char ackEvent;  // to send a command to remotes with a confirmation response
   char ackData; // maybe not used
   char m2m_trans;
};
...
{  char active;
	char MAC[6];
   long ipAddr;
   char staConfig;  // station number or diverter configuration number (aka port mapping)
   char devType;    // remote, diverter, slave, etc...
   char CIC;
   char Door;
   char Arrival;
   char RTS;
   char RTS2;
   char diverterStation;  // contains station num from Payload.diverterStation when ready
   char headDivAlign;
   char version;
   char subversion;
   char ready4Trans;
   char ackEvent;  // return response to ack Payload.ackEvent
   char ackCommand;
   char lastSequence; // rolling packet sequence number
   char flags1;  // reserved for device specific data
   char flags2;  // reserved for device specific data
};
struct device_discovery_type
{  char MAC[6];
   long ipAddr;
   char devType;
   char system;
   char staConfig;  // station number for remotes or diverter config
   unsigned long delay;
};
// Define communications using TCP/IP (UDP)
   ...
   #define MY_BASE_IP "192.168.%d.10"
   char myIPaddress[18];  // string to hold runtime IP address
	#define MY_IP_NETMASK "255.255.255.0"
	#define PORT_TO_SERVER   1236
	//#define REMOTE_IP    "0.0.0.0" //255.255.255.255" /*broadcast*/
	#define PORT_FROM_SERVER 1236
   #define PORT_TO_REMOTE   1234
   #define PORT_FROM_REMOTE 1235
	#use "dcrtcp.lib"
	udp_Socket serverSocket;  // Socket for server communications
   udp_Socket remoteSocket;  // Socket for remote communications
   long lastRemIP;  			  // IP and port of last incoming message
   int  lastRemPort;
	int sendUDPHeartbeat(char sample);
	int sendInputsToMain(char sample);
	int sendServerCommand(struct ServerMessageType message);
	int sendRemoteCommand(struct RemoteMessageType message, int destDevice);
   #define DEV_BROADCAST -1
   #define DEV_SENDALL -2
   #define DEV_SENDSLAVE -3
	int sendOneRemoteCommand(struct RemoteMessageType message, long ipAddr);
	int getRemoteMessage(struct RemoteMessageType *message);
	char sendAckMessage(struct RemoteMessageType message, unsigned long TODuration);
	void processRemoteMessage(struct RemoteMessageType message);
   ...
   void sendPayloadToRemotes(void);
...
{  // Define all parameters that are saved in flash
   ...
   char cardNumBits;       // number of bits read from the ID card
   char cardSiteStart;     // location for the start of the site code
   char cardSiteLength;    // length of the site code
   char cardIdStart; 		// location for the start of the id number
   char cardIdLength;  	   // length of the id number
   char disableLocks;      // bypass door lock activation
   int cardSiteCode;       // acceptable card site code (0 = any)
...
char msgSequenceS, msgSequenceR;
...
#define MAX_DEVADDRS  14   // maximum number of remote devices
...
unsigned long lastComTime; // to send payload to remotes on a set frequency
...
void slaveProcessCommand(struct RemoteMessageType message);
...
char slaveGetCommand(struct RemoteMessageType *message);
void slaveSendResponse(struct RemoteMessageType message, long ipAddr);
...
MYDEBUG int station2bit(char station);
MYDEBUG char firstBit(char bitdata);
MYDEBUG char firstBit_i(int bitdata);
...
void set_remote_diverters(char station);
int  ready4dispatch(void);
char confirmAckEvent(void);
char confirmAckCommand(char ackCmd);
...
MYDEBUG void startupRegisterDevices(void);
...
struct PayloadType Payload;  // data package for remotes
char secureAck;
...
char remote_data[5];
struct device_info_type deviceInfo[MAX_DEVADDRS];
...
void main()
{
   ...
   struct RemoteMessageType UDPmessage;
   ...
   //if(sock_init()) printf("IP sock_init() failed\n");	// Initialize UDP communications
	ifconfig(IF_ETH0,
	         ...
	         IFS_END);
   // Open Server Socket
   if(!udp_open(&serverSocket, PORT_TO_SERVER, -1/*resolve(REMOTE_IP)*/, PORT_FROM_SERVER, NULL))
   {  printf("udp_open server failed!\n");
   }
   // Open Remote Socket
   if(!udp_open(&remoteSocket, PORT_TO_REMOTE, -1/*resolve(REMOTE_IP)*/, PORT_FROM_REMOTE, NULL))
   {  printf("udp_open remote failed!\n");
   }
   ...
   // Perform bulb check
   lcd_DispText("Checking Indicators ...\n",0, 0, MODE_NORMAL);
   exercise_outputs();

   // register devices
   startupRegisterDevices();
   ...
   if (param.slaveController==0)
   {
	   while(1)
	   {
         ...
         if (getRemoteMessage(&UDPmessage) > 0) processRemoteMessage(UDPmessage);
...
{  // Setup system i/o processing routine
   ...
   availableDevices=0; // processing of commands will pick this up
   ...
   {  secureTransLog[i].sid=0;
   	secureTransLog[i].trans_num=0;
   	secureTransLog[i].start_tm=0;
      securePIN[i]=-1;
   }

   // initialize device info structure
   for (i=0; i<MAX_DEVADDRS; i++)
   {  deviceInfo[i].active = 0;
   	deviceInfo[i].ipAddr = 0;
   }
...
{  struct RemoteMessageType command;
   command.command=CANCEL_PENDING;
   sendRemoteCommand(command, DEV_SENDSLAVE);
}
...
//   int safetyStatus;
   ...
   #GLOBAL_INIT
   {
      ...
      Payload.systemState=system_state;
      Payload.mainArrival=0;
      Payload.remoteAlert=0;
      Payload.transStation=0;
      Payload.diverterStation=0;
      Payload.ackEvent=0;
	   Payload.purgeMode=FALSE;
      Payload.m2m_trans=0;
...
/*   if (secureAck)
   ...
   // update I/O based on return of inputs
   // DO WE NEED TO THROTTLE THE PROCESSING OF INPUTS (EG EVERY X MSEC)
   if (1) //send_n_get(command, &response))
   {
      if (1) //response.command==INPUTS_ARE)
      {
	      // store slave response in local vars
	      if (slaveAvailable)
	      {  // slaveAvailable is the device index into deviceInfo
         	if (deviceInfo[slaveAvailable].CIC)      // slave carrier in chamber
	         {   slave_cic = TRUE;
	         ...
	         if (deviceInfo[slaveAvailable].Door)      // slave door
            ...
            if (deviceInfo[slaveAvailable].Arrival)		// show message if slave optic blocked
				{  message_add(SYSTEMLCD, MSG_OPTIC_BLOCKED, MSG_AT+SLAVE, NEXT);
            } else
            {  message_del(SYSTEMLCD, MSG_OPTIC_BLOCKED, MSG_AT+SLAVE);
            }
            if (deviceInfo[slaveAvailable].flags1 & 0x08) btnStatFlag=1;  // Set stat from slave
            if (deviceInfo[slaveAvailable].flags1 & 0x10) btnStatFlag=2;  // clear stat from slave
            if (deviceInfo[slaveAvailable].flags1 & 0x20) btnSecureFlag=1;  // Set secure from slave
            if (deviceInfo[slaveAvailable].flags1 & 0x40) btnSecureFlag=2;  // clear secure from slave
	         slave_rts = deviceInfo[slaveAvailable].RTS;     // slave request to send
	         slaveReturnStatus = deviceInfo[slaveAvailable].flags2;  // slave status
...
//      message_add(SYSTEMLCD, MSG_PIN, MSG_AT+systemStation);
   ...
   // Handle processing of system state
   switch (system_state)
   {
      case IDLE_STATE:
...
/// why??      if (lcd_autoReturn() || autoRtnTrans) translog.flags = FLAG_ARETURN;
         ...
         {  // setup to process a transaction request
            ...
            Payload.m2m_trans = main2main_trans;
      ...
      case PREPARE_SEND:      // does job for send and return
         ...
         Payload.diverterStation=systemStation;
         Payload.transFlags=translog.flags;
         Payload.securePin = securePIN[systemStation];
         Payload.headDivAlign=transTypeInfo[transactionType][TT_FROM];
         set_remote_diverters(systemStation);
         //if (received) new_state=WAIT_FOR_DIVERTERS;
         //else new_state=CANCEL_STATE;
         new_state=WAIT_FOR_DIVERTERS;
         break;
      ...
      case BEGIN_SEND:          // works for both directions
         ...
         Payload.transStation=systemStation;
         Payload.mainStation=mainStation;
         Payload.transDirection=system_direction;

         // are we ok to send?
         if (command.data[0] == TRUE)
         {
            // make sure remotes are ready
            if (ready4dispatch() == availableDevices)
            {
               // turn on blower
            ...
            {  // remote door?
              /// Make no assumptions but if door it will be caught in next state
      ...
      case WAIT_FOR_REM_ARRIVE:
       ...
       {  // handle arrival, transaction complete
          ...
          Payload.ackEvent=TRANS_COMPLETE;
      ...
      case WAIT_FOR_HEADDIVERTER:
       ...
       // wait for arrival at headdiverter controller
	      if (remote_data[REMOTE_ARRIVE] & HEADDIVERTERSTATION) // at headdiverter
	      {
            ...
            new_state=SHIFT_HEADDIVERTER;
	         //else new_state=CANCEL_STATE;
	         Payload.headDivAlign=transTypeInfo[transactionType][TT_TO];
	      }
      ...
      case WAIT_FOR_MAIN_ARRIVE:
         ...
         {  // handle arrival, transaction complete
            ...
            {  if (translog.flags & FLAG_ARETURNING) main_arrival=1;  // constant on alert
               ...
               Payload.mainArrival=systemStationb;
            }
            ...
            //finalcmd.command=TRANS_COMPLETE;
            //finalcmd.station=systemStation;
            Payload.remoteAlert=0; // no remote alert
...
*/
      ...
      case MALFUNCTION_STATE:
        ...
        secureTrans = FALSE;
        autoRtnTrans = FALSE;
        // signal alarm at remote -- done by system_state
        //command.devAddr=ALL_DEVICES;
   ...
   if (new_state != system_state)
   {
      ...
      Payload.systemState=new_state;
      first_time_here=TRUE;    // wasn't here yet
      sendUDPHeartbeat(0);     // notify host on state change
	   sendPayloadToRemotes();  // notify remotes on state change
   }
...
{  // Look into secure tracking structure for any removed secure transactions
   ...
   for (i=1; i<9; i++)
   {  // anything to do?  when sid and trans_num and start time are non zero
      if (secureTransLog[i].sid && secureTransLog[i].start_tm && secureTransLog[i].trans_num)
      {  // handle secure removal
         ...
         addTransaction(*(struct trans_log_type *)&secureTransLog[i]);
      	// clear PIN message
         message_del(SYSTEMLCD, MSG_PIN, MSG_AT+i);
         // acknowledge it ... again (WHY?)
...
/******************************************************************/
int ready4dispatch(void)
{  // makes sure all remote devices say they are ready
   // returns the device set for all ready devices
   int i, rtnval;
// RETURN VALUE NEEDS TO BE INTEGER
   rtnval=0;
   for (i=1; i<MAX_DEVADDRS; i++)
   {  if (deviceInfo[i].active)
      { if (deviceInfo[i].ready4Trans == Payload.transStation)
      	{ rtnval |= station2bit(i); }  // set this bit in the return value
      }
   }
   return rtnval;
}
char confirmAckEvent(void)
{  // makes sure all remote devices have acknowledged the ackEvent
   // typically used in a non-blocking looping process to ensure events are received
   // returns true or false
   char i, rtnval;

   rtnval = TRUE; // assume the test will pass
   for (i=1; i<MAX_DEVADDRS; i++)
   {  if (deviceInfo[i].active)
      { if (deviceInfo[i].ackEvent != Payload.ackEvent)
      	{ rtnval = FALSE; }  // set this bit in the return value
      }
   }
   if (rtnval == TRUE) Payload.ackEvent=0;  // Clear command when all have received
   return rtnval;
}
char confirmAckCommand(char ackCmd)
{  // makes sure all remote devices have acknowledged the ackCommand
   // ackCmd = command being confirmed
   // typically used in a blocking process to ensure command messages are received
   // returns true or false
   char i, rtnval;
   ...
   for (i=1; i<MAX_DEVADDRS; i++)
   {  if (deviceInfo[i].active)
      { if (deviceInfo[i].ackCommand != ackCmd)
      	{ rtnval = FALSE; }  // set this bit in the return value
      }
   }
   return rtnval;
...
// LOOKS AT CURRENT STATUS OF REMOTE & DIVERTER SIGNALS TO SEE IF ALL ARE READY
   char i, rtnval;

   // remote diverter info now comes in deviceInfo structure
   rtnval=TRUE;  // Assume good
   diverter_status = 0;  // clear
   for (i=1; i<MAX_DEVADDRS; i++)
   {  if (deviceInfo[i].active)
      { if ((deviceInfo[i].diverterStation == Payload.diverterStation)
           && (deviceInfo[i].headDivAlign == Payload.headDivAlign))
             { diverter_status |= station2bit(i); }
        else { rtnval=FALSE; }
      }
   }
// HOW DO WE MAKE SURE LOCAL DIVERTER IS SET?
   // setup the diverter status to find out which device is not ready
   if ((diverter_map[station]==0) || (di_diverterPos==diverter_map[station]))
        {  diverter_status |= 0x100; }
   else { rtnval=FALSE; }
...
void set_remote_diverters(char station)
{  // sends command to remote systems to align diverters
   // NOW USE payload.diverterStation TO SET DIVERTERS
};
...
void startupRegisterDevices()
{  // detect and register remote devices
   ...
   //strcpy(msgbuf, "#n WD:___ PL:___");
	lcd_DispText("Checking Devices ...\n", 0, 0, MODE_NORMAL);
   // run for 1 second to allow devices to check in
   delayTimer = MS_TIMER;
   while (MS_TIMER - delayTimer < 1000)
   {	// Process messages for a while
  		tcp_tick(NULL);
      if (getRemoteMessage(&UDPmessage) > 0) processRemoteMessage(UDPmessage);
   }
   // now send everyone talking to me a device info request
   // issue trigger to retrieve device information
   UDPmessage.command = DEVICE_INFO;
   sendRemoteCommand(UDPmessage, DEV_SENDALL);
   delayTimer = MS_TIMER;
   while (MS_TIMER - delayTimer < 1000)
   {	// Process messages for a while
  		tcp_tick(NULL);
   ...
   for (i=0; i<MAX_DEVADDRS; i++)
   {  if (deviceInfo[i].active)
      {  sprintf(msgbuf, "DEV %d.%02d VER %d.%02d %2X%2X%2X %-15.15s\n",
      		deviceInfo[i].devType, deviceInfo[i].staConfig,
      		deviceInfo[i].version, deviceInfo[i].subversion,
            deviceInfo[i].MAC[3], deviceInfo[i].MAC[4], deviceInfo[i].MAC[5],
   	      inet_ntoa(ipbuf, deviceInfo[i].ipAddr));
...
const struct{
   ...
   char partype;  // parameter type (flag, clock, integer value, other)
	char parlen;   // how many bytes for numeric parameters
 ...
 MNU_MARRIVAL_BEEP,  "BEEP ON MAIN ARRIVE",MNU_FLAG, 1, &param.main_arrive_alert, 0, 0, "",
 MNU_RARRIVAL_BEEP,  "BEEP ON REMT ARRIVE",MNU_FLAG, 1, &param.remote_arrive_alert, 0, 1, "",
 MNU_ARETURN_BEEP,   "BEEP ON AUTO RETURN",MNU_FLAG, 1, &param.areturn_arrive_alert, 0, 1, "",
 MNU_ALARM_SOUND,    "SILENCE ALARM",      MNU_FLAG, 1, &alarm_silenced_flag, 0, 0, "",
 MNU_PURGE,          "MANUAL PURGE",       MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_ALARM_RESET,    "RESET ALARM",        MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_SETACTIVEMAIN,  "SET ACTIVE MAIN",    MNU_OTHR, 1, &activeMainHere, 0, 0, "",
 MNU_SET_CLOCK, 		"SET CLOCK",          MNU_CLK,  0, &NOch, 0, 0, "",
 MNU_COMM_STAT,      "SHOW COMMUNICATIONS",MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_MSTACKING,      "STACK AT MAIN",      MNU_FLAG, 1, &param.stacking_ok[0], 0, 1, "",
 MNU_RSTACKING,      "STACK AT REMOTES",   MNU_FLAG, 1, &param.stacking_ok[1], 0, 1, "",
 MNU_RESET_COUNT,    "SET TRANS COUNT",    MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_SET_STANAMES,   "SET STATION NAMES",  MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_SET_PHONENUMS,  "SET PHONE NUMBERS",  MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_DIVERTER_CFG,   "USE LOCAL DIVERTER", MNU_FLAG, 1, &param.localDiverter, 0, 1, "",
 MNU_SETPASSWORD,    "SET PASSWORD",       MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_CLEARPASSWORDS, "CLEAR PASSWORDS",    MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_SETTIMEOUT,     "DELIVERY TIMEOUT",   MNU_VAL,  2, (char *)&param.deliveryTimeout, 10, 240, "SEC",
 MNU_HEADDIVERTER,   "USE HEAD DIVERTER",  MNU_FLAG, 1, &param.headDiverter, 0, 1, "",
 MNU_AUTO_PURGE,     "AUTOMATIC PURGE",    MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_AUTOPUR_TIMER,  "AUTO PURGE TIMER",   MNU_VAL,  2, (char *)&param.autoPurgeTimer, 10, 200, "SEC",
 MNU_SETASSLAVE,     "SET AS SLAVE DEVICE",MNU_FLAG, 1, &param.slaveController, 0, 1, "",
 MNU_DEF_STATIONS,   "SET ACTIVE STATIONS",MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_ADVANCED_SETUP, "Only in old menu",   MNU_FLAG, 1, &diagnostic_mode, 0, 0, "",
 MNU_RESET,          "RESTART SYSTEM",     MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_ADMIN_FEATURES, "ADMIN FEATURES",     MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_MAINT_FEATURES, "MAINT FEATURES",     MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_SYSTEM_CONFIG,  "SYSTEM CONFIG",      MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_VIEW_LOGS,      "VIEW LOGS",          MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_VIEW_TLOG,      "VIEW TRANSACTIONS",  MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_VIEW_SLOG,      "VIEW SUMMARY LOG",   MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_VIEW_ELOG,      "VIEW EVENT LOG",     MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_VIEW_ALOG,      "VIEW RECENT ALARMS", MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_AUTO_RETURN,    "AUTO RETURN TIMER",  MNU_VAL,  2, (char *)&param.autoReturnTimer, 0, 60, "MIN",
 MNU_DNLD_TLOG,      "DOWNLOAD TRANS LOG", MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_UPLD_TLOG,      "UPLOAD TRANS LOG",   MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_CHECK_SFLASH,   "CHECK SERIAL FLASH", MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_ANALYZE_LOG,    "ANALYZE EVENT LOG",  MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_SUBSTA_ADDR,    "SUB-STA ADDRESSING", MNU_FLAG, 1, &param.subStaAddressing, 0, 1, "",
 MNU_BLOWER_TYPE,    "SET BLOWER TYPE",    MNU_OTHR, 1, &param.blowerType, 0, 0, "",
 MNU_PORT_MAPPING,   "SET PORT MAPPING",   MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_LCD_CALIBRATE,  "CALIBRATE SCREEN",   MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_SET_SYSNUM,     "SET SYSTEM NUMBER",  MNU_VAL,  1, &param.systemNum, 1, 20, "",
 MNU_SHOW_IPADDR,    "SHOW IP ADDRESS",    MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_SECURE_SETUP,   "SETUP SECURE TRANS", MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_SECURE_ENABLE,  "ENABLE SECURE TRANS",MNU_FLAG, 1, &param.secureEnabled, 0, 1, "",
 MNU_CARD_NUMBITS,   "CARD NUMBER OF BITS",MNU_VAL,  1, &param.cardNumBits, 26, 64, "ID",
 MNU_CARD_SITE,      "CARD SITE CODE",     MNU_VAL,  2, (char *)&param.cardSiteCode, 0, 9999, "ID",
 MNU_SECURE_L3DIGITS,"CARD LEFT 3 DIGITS", MNU_VAL,  1, &param.cardL3Digits, 154, 154, "ID",
 MNU_SECURE_R9MIN,   "CARD RIGHT 9 MIN",   MNU_VAL,  4, (char *)&param.cardR9Min, 200000000, 618000000, "ID",
 MNU_SECURE_R9MAX,   "CARD RIGHT 9 MAX",   MNU_VAL,  4, (char *)&param.cardR9Max, 200000000, 618000000, "ID",
 MNU_SHOW_INPUTS,    "SHOW INPUTS",        MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_TUBE_DRYER,     "RUN TUBE DRYER",     MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_DISABLE_LOCKS,  "DISABLE DOOR LOCKS", MNU_FLAG, 1, &param.disableLocks, 0, 1, "",
 MNU_REGISTER_DEV,   "REGISTER DEVICES",   MNU_OTHR, 0, &NOch, 0, 0, "",
 MNU_LAST,           "END",                MNU_NONE, 0, &NOch, 0, 0, "" };
...
void setupMenuOrder(char menuLevel)
{
   ...
   case 6: // secure card setup
      ...
      menuOrder[i++]=MNU_CARD_NUMBITS;
		menuOrder[i++]=MNU_CARD_SITE;
...
{  // can modify menu_level using advanced configuration menu item
   ...
   case MNU_VAL:
      ...
      if (menu[menu_idx].parlen == 4) //max > 999)
      {  // unsigned long data type
	      maxDigits = 9;  // 9 digits max
	      temp_ulong = *(unsigned long*)menu[menu_idx].parval; // type is ulong
      }
      else if (menu[menu_idx].parlen == 2)
      {  // integer data type
	      maxDigits = 5;  // 5 digits max
	      temp_ulong = *(int *)menu[menu_idx].parval; // type is integer
      }
      else
      {  // character data type
      ...
      {
      	if (menu[menu_idx].parlen == 4) // max > 999)  // cast unsigned long
         	*(unsigned long*)menu[menu_idx].parval = temp_ulong;
	      else if (menu[menu_idx].parlen == 2) // cast integer
         	*(int *)menu[menu_idx].parval = (int)temp_ulong;
			else                           // cast character
...
void purgeSystem(char purge_mode)
{
   ...
   Payload.systemState=blower_purge_state;
   ...
   {
      //hitwd();          // in maintenance()    // hit the watchdog timer
      //showactivity();   // in maintenance()
      maintenance();
      if (getRemoteMessage(&UDPmessage) > 0) processRemoteMessage(UDPmessage);
      check_diverter();
      // processBlower(); // in maintenance()
      ...
      case PURGE_SET_DIVERTER:  // set diverter and continue
         ...
         set_diverter(systemStation);
         Payload.diverterStation=systemStation;
         Payload.headDivAlign=align_headdiverter;

         //if (send_command(command)) new_state=PURGE_WAIT_DIVERTER;
         new_state=PURGE_WAIT_DIVERTER;
         break;
      ...
      case PURGE_RUN:
         ...
         if ((remote_data[REMOTE_ARRIVE] & HEADDIVERTERSTATION)
            && (system_direction==DIR_RETURN))
         {
...
{  // returns 0 if no error, non-zero otherwise
   ...
   *(unsigned long *)&command.data[1] = param.cardR9Max;
   ...
   ///msDelay(10);
	...
	UDPmessage.command=SET_PARAMETERS;
	UDPmessage.station=0;
   UDPmessage.data[0]=param.blowerType;
// FIX HOW TO SEND PORT MAPPING
//   for (i=1; i<MAX_DEVADDRS; i++) UDPmessage.data[i]=param.portMapping[i];
// MAYBE DON'T NEED TO IF IT'S HANDLED BY DEVICE REGISTRATION PROCESS
	UDPmessage.data[10]=param.cardNumBits; // ID card number of bits
	UDPmessage.data[11]=param.cardSiteStart; // ID card site code start
	UDPmessage.data[12]=param.cardSiteLength; // ID card site code length
	UDPmessage.data[13]=param.cardIdStart; // ID card ID number start
  	UDPmessage.data[14]=param.cardIdLength; // ID card ID number length
   UDPmessage.data[15]=param.disableLocks; // bypass door locks
   *(int *)&UDPmessage.data[16] = param.cardSiteCode;
   // send this message in a loop (just send once) until it is acknowledged by all
   if (sendAckMessage(UDPmessage, 1000)) status=0;
   else status=1;  // no good

   return status;
}
char sendAckMessage(struct RemoteMessageType message, unsigned long TODuration)
{  // send the message periodically until all devices have acknowledged
   // timeOut in milliseconds
   char allAcked, i;
   unsigned long TOStart;
   unsigned long interval;
   unsigned long sendAgain;
   struct RemoteMessageType UDPmessage;

   allAcked=FALSE;
   interval=100; // send message interval in msec
   sendAgain=MS_TIMER;
   sendRemoteCommand(message, DEV_SENDALL);
   TOStart=MS_TIMER;
   while ((allAcked==FALSE) && (Timeout(TOStart, TODuration)==FALSE))
   {  // handle maintenance items
	   lastComTime = MS_TIMER; // delay sending of payload
      maintenance();
      if (getRemoteMessage(&UDPmessage) > 0) processRemoteMessage(UDPmessage);

      // send every X msec - Make send interval a parameter
      ///if ((MS_TIMER-sendAgain) > interval)
		///{  sendRemoteCommand(message);
      ///	sendAgain=MS_TIMER+interval;
      ///}

	   // until all have acked
      allAcked=confirmAckCommand(message.command);
   }
   return allAcked;
}
...
{  // handle all regularly scheduled calls
   ...
   //rn_keyProcess(DevRN1600, 0);  // process keypad device
	...
	// send payload data to remotes periodically
	if (MS_TIMER - lastComTime > 100)
	{  lastComTime = MS_TIMER;
	   sendPayloadToRemotes();
   }
...
char firstBit_i(int bitdata)
{  // Return # of first bit that is on (1-8)
   char i;
   i=0;
   while (i<16 && ((1<<i & bitdata)==0)) i++;
   if (i<16) return i+1;
   else     return 0;
}
...
*********************************************************/
int getRemoteMessage(struct RemoteMessageType *UDPmessage)
{
   ...
   length = sizeof(buf);
   retval = udp_recvfrom(&remoteSocket, buf, length, &(longword)lastRemIP, &(word)lastRemPort);
   if (retval < -1) {
      printf("Error %d receiving from remote!  Closing and reopening socket...\n", retval);
      sock_close(&remoteSocket);
      if(!udp_open(&remoteSocket, PORT_TO_REMOTE, -1/*resolve(REMOTE_IP)*/, PORT_FROM_REMOTE, NULL)) {
         printf("udp_open remote failed!\n");
      }
   }
   else if (retval > 0)
   {
      ...
      /// if message.device = param.systemNumber then it is for me
...
int sendRemoteCommand(struct RemoteMessageType UDPmessage, int destDevice)
{  // sends the command to one or more destinations based on destDevice
	// destDevice = -1 = Broadcast to all
   //            = -2 = Send individually to each
   //            = 0..n = Send only to device n within deviceInfo structure
   //   optionally could specify device type for targeted groups

	long ipAddr;
   int idx;

   if (destDevice == DEV_BROADCAST)
   {  // broadcast
   	ipAddr = -1;
   	sendOneRemoteCommand(UDPmessage, ipAddr);
   }
   else if (destDevice > 0 && destDevice < MAX_DEVADDRS)
   {  // single device
   	if (deviceInfo[destDevice].active)
   	{  ipAddr = deviceInfo[destDevice].ipAddr;
	   	sendOneRemoteCommand(UDPmessage, ipAddr);
      }
   }
   else if (destDevice == DEV_SENDALL)
   {  // all devices
   	for (idx=0; idx<MAX_DEVADDRS; idx++)
	   {  if (deviceInfo[idx].active)
	      {  ipAddr = deviceInfo[idx].ipAddr;
	         sendOneRemoteCommand(UDPmessage, ipAddr);
	      }
	   }
	}
   else if (destDevice == DEV_SENDSLAVE)
   {  // just to the slave device (type 5)
   	for (idx=0; idx<MAX_DEVADDRS; idx++)
      {  if (deviceInfo[idx].active && (deviceInfo[idx].devType == 5))
	      {  ipAddr = deviceInfo[idx].ipAddr;
	         sendOneRemoteCommand(UDPmessage, ipAddr);
	      }
	   }
...
int sendOneRemoteCommand(struct RemoteMessageType UDPmessage, long ipAddr)
{
   ...
   UDPmessage.system = param.systemNum;  // same
   UDPmessage.devType = 1; // main
   UDPmessage.sequence = ++msgSequenceR;
   ...
   /* send the packet */
   retval = udp_sendto(&remoteSocket, UDPbuffer, length, ipAddr, PORT_FROM_REMOTE);
   if (retval < 0) {
      printf("Error sending to remote!  Closing and reopening socket...\n");
      sock_close(&remoteSocket);
      if(!udp_open(&remoteSocket, PORT_TO_REMOTE, -1/*resolve(REMOTE_IP)*/, PORT_FROM_REMOTE, NULL)) {
         printf("udp_open remote failed!\n");
      }
...
// Sends secure card parameters the remote stations
	struct RemoteMessageType HBmessage;

   HBmessage.command = SET_CARD_PARAMS;
   HBmessage.station = 0; //MY_STATIONS;
   ...
   *(unsigned long*)&HBmessage.data[5] = param.cardR9Max; //
   sendRemoteCommand(HBmessage, DEV_SENDALL); // ignore return value, will send again periodically
}
int sendServerCommand(struct ServerMessageType UDPmessage)
{
	// Send the supplied command over UDP
   // Appends some info to the standard message
   //   .device
   ...
   // fill the packet with additional data
   UDPmessage.system = param.systemNum;  // system id
   UDPmessage.device = param.systemNum;  // system id as device for now
   UDPmessage.devType = 1; // main station
   UDPmessage.sequence = ++msgSequenceS;
   UDPmessage.timestamp = MS_TIMER;
   ...
   /* send the packet */
   retval = udp_send(&serverSocket, UDPbuffer, length);
   if (retval < 0) {
      printf("Error sending to server!  Closing and reopening socket...\n");
...
int sendUDPHeartbeat(char sample)
{  // Sends an INPUTS_ARE message to the server

	struct ServerMessageType HBmessage;
   ...
   sendServerCommand(HBmessage); // ignore return value, will send again periodically
}
void sendUDPtransaction(struct trans_log_type translog )
{
	struct ServerMessageType TLmessage;
   ...
   if (translog.status <= LAST_TRANS_EVENT)
	...
	{  TLmessage.command = ESECURE_REMOVAL;
      ...
      // add 5th byte at first data element past the end of the translog
      if (param.cardNumBits == 40)
	        TLmessage.data[sizeof(translog)] = param.cardL3Digits-CARDHBOFFSET; // add on 5th byte of 38 bit card id
      else TLmessage.data[sizeof(translog)] = 0;
   }
   ...
   sendServerCommand(TLmessage); // ignore return value, will send again periodically
}

void sendParametersToLogger(void)
{  // send the full parameter set over TCP/IP UDP
	struct ServerMessageType PBmessage;
   char * PBbuffer;  // to index into translog byte by byte
   int idx;

   if (sizeof(param) > NUMUDPDATA) printf("NUMUDPDATA UDP BUFFER TOO SMALL %d should be %d\n", NUMUDPDATA, sizeof(param));
   ...
   sendServerCommand(PBmessage); // ignore return value, will send again periodically

}

void sendPayloadToRemotes(void)
{  // sends the standard message buffer to the remote systems

   auto int length, retval;
   char * UDPbuffer;  // to index into UDPmessage byte by byte

   // fill the packet with additional data
   Payload.system = param.systemNum;
   Payload.device = param.systemNum;  // System number
   Payload.devType = 1;  // MAIN
   Payload.command = REMOTE_PAYLOAD;
   Payload.sequence = ++msgSequenceR;
   Payload.timestamp = MS_TIMER;

   length = sizeof(Payload); // how many bytes to send
   UDPbuffer = (char *) &Payload;

   /* send the packet */
   retval = udp_send(&remoteSocket, UDPbuffer, length);
   if (retval < 0)
   {  printf("Error sending to remote!  Closing and reopening socket...\n");
      sock_close(&remoteSocket);
      if(!udp_open(&remoteSocket, PORT_TO_SERVER, -1/*resolve(REMOTE_IP)*/, PORT_FROM_SERVER, NULL)) {
         printf("udp_open failed!\n");
      }
   }
   else
   {  // some data is one-shot and needs to be cleared after sending
		Payload.secureAckStation=0;
   }
}
char registerThisDevice(struct RemoteMessageType message)
{  // look for this device in the device structure and register it if its not there
	char i;
   char match;
   struct RemoteMessageType outMessage;
   // first check if it exists
   match = 0;
   i=1;
   while ((i<MAX_DEVADDRS) && (match==0))
   {  //if ((deviceInfo[i].active==TRUE) && (lastRemIP == deviceInfo[i].ipAddr)) match = i;
   	// don't care if active or not, just match based on IP
   	if (lastRemIP == deviceInfo[i].ipAddr) match = i;
      i++;
   }

   // if it didn't match then allocate a slot
   if (match==0)
   {  i=1;
   	while ((i<MAX_DEVADDRS) && (match==0))
   	{  // find first open slot
      	if (deviceInfo[i].active==FALSE)
         {  // use this slot
         	match=i;
         }
         i++;
   ...
   // if match still zero then we have too many devices or a bug
   if (match==0) printf("ERROR registering this device %d\n", message.device);

   outMessage.command = REGISTER_DEVICE;
   outMessage.data[0] = 0; // don't know it yet //device.staConfig;  // station or diverter configuration
   outMessage.data[1] = match;               // device number
   sendRemoteCommand(outMessage, match);

   return match;
}
void processRemoteMessage(struct RemoteMessageType UDPmessage)
{
	// Handle any incoming UDP packets for me
   struct ServerMessageType ServerMessage;
   int b_station;
   static unsigned long print_timer;
   unsigned long secureID;
   static int msg_count;
   static unsigned long lastComCheck;
   static int recentDevices; // to track active communication
   char thisDevice;
   #define chkTimer 2
   int i;
   #GLOBAL_INIT { msg_count=0; lastComCheck=0; }
   ...
   thisDevice = UDPmessage.device;
   if (thisDevice > MAX_DEVADDRS) thisDevice = 0;  // zero is the invalid device
   if (deviceInfo[thisDevice].active == FALSE) thisDevice = 0; // put into next queue slot

   // if thisDevice is 0 then need to register it and flag as active
   if (thisDevice==0) thisDevice = registerThisDevice(UDPmessage);
	deviceInfo[thisDevice].active = TRUE;
   deviceInfo[thisDevice].ipAddr = lastRemIP;

   // show that this device is talking
   availableDevices |= station2bit(thisDevice);  // register this device
   recentDevices |= station2bit(thisDevice);
   if ((SEC_TIMER - lastComCheck) == chkTimer)
   {
      if (recentDevices != availableDevices)
      {  comError = firstBit_i(availableDevices &~ recentDevices); }  // sets device number, not bit
      else { comError = 0; }
      lastComCheck=SEC_TIMER;
      recentDevices=0;
   } else if ((SEC_TIMER - lastComCheck) >= chkTimer)
   {  // must be out of range so start over
      lastComCheck=SEC_TIMER;
      recentDevices=0;
      comError=0;
   }
   ...
   {
      case DEVICE_INFO:
         //deviceInfo[UDPmessage.device].MAC = *(unsigned long*)&UDPmessage.data[0];
         // also capture MAC and ipAddress
         for (i=0; i<6; i++) deviceInfo[thisDevice].MAC[i] = UDPmessage.data[i];
         // capturing IP in line below is redundant but if they differ then there is a problem
         if (deviceInfo[thisDevice].ipAddr != *(unsigned long*)&UDPmessage.data[4])
         	printf("IP mismatch %ld <> %ld\n", deviceInfo[thisDevice].ipAddr, *(unsigned long*)&UDPmessage.data[4]);
         deviceInfo[thisDevice].ipAddr = *(unsigned long*)&UDPmessage.data[4];
         deviceInfo[thisDevice].staConfig = UDPmessage.data[16];
         deviceInfo[thisDevice].devType = UDPmessage.devType;
			if (UDPmessage.devType == 5) slaveAvailable=thisDevice;
      ...
      case INPUTS_ARE:
         ...
         deviceInfo[thisDevice].diverterStation = UDPmessage.data[14];
         deviceInfo[thisDevice].headDivAlign = UDPmessage.data[15];
			deviceInfo[thisDevice].ackEvent = UDPmessage.data[17];
         deviceInfo[thisDevice].ackCommand = UDPmessage.data[18];
         deviceInfo[thisDevice].ready4Trans = UDPmessage.data[19];
         deviceInfo[thisDevice].version = UDPmessage.data[5];
         deviceInfo[thisDevice].subversion = UDPmessage.data[6];
         deviceInfo[thisDevice].flags1 = UDPmessage.data[7];
         deviceInfo[thisDevice].flags2 = UDPmessage.data[8];
         STATION_SET |= b_station;  // include this station in the processing
         ...
         if (UDPmessage.devType == 4) // stuff for head diverter
         {  blwrError = (UDPmessage.data[4] == 0xFF); }
         else
         {  // non head diverter code
         ...
         {  // non-zero ID to be cached
	         secureTransLog[UDPmessage.station].sid = secureID;
	         secureTransLog[UDPmessage.station].flags = UDPmessage.data[13]; // scaled time
	         // tell remote we got the card ID
            Payload.secureAckStation = UDPmessage.station;
         }
      	break;

      case CARD_SCAN:
      	// pass the card scan up to the server
         ServerMessage.command = UDPmessage.command;
         ServerMessage.station = UDPmessage.station;
         // take raw data from 0 to 9
         for (i=0; i<10; i++)
	         ServerMessage.data[i] = UDPmessage.data[i];
         // append clock timer to the end
         *(unsigned long*)&ServerMessage.data[10] = SEC_TIMER;
         sendServerCommand(ServerMessage);
         break;
   }
}
...
/********************************************************/
void slaveSendResponse(struct RemoteMessageType UDPmessage, long ipAddr)
{
	// Send the supplied command over UDP
   // Appends some info to the standard message
   //   .device
   ...
   // fill the packet with additional data
   UDPmessage.system = param.systemNum;  // system id
   UDPmessage.device = SLAVE_DEVADDR;  // device id
   UDPmessage.devType = 5; // slave
   UDPmessage.sequence = ++msgSequenceR;
   UDPmessage.timestamp = MS_TIMER;
   ...
   /* send the packet */
   retval = udp_sendto(&remoteSocket, UDPbuffer, length, ipAddr, PORT_TO_REMOTE);
   if (retval < 0) {
      printf("Error sending datagram!  Closing and reopening socket...\n");
      sock_close(&remoteSocket);
      if(!udp_open(&remoteSocket, PORT_FROM_REMOTE, -1/*resolve(REMOTE_IP)*/, PORT_TO_REMOTE, NULL)) {
         printf("udp_open failed!\n");
      }
   }
...
/********************************************************/
char slaveGetCommand(struct RemoteMessageType *UDPmessage)
{
/* Repeatedly call this function to process incoming commands efficiently. */
   ...
   length = sizeof(buf);
   retval = udp_recvfrom(&remoteSocket, buf, length, &(longword)lastIPaddr, &(word)lastPort);
   if (retval < -1) {
...
void show_comm_status()
{
   ...
   if (SEC_TIMER > updateclock)
   {
      ...
      if (comError)
      {  sprintf(eventMsg, "%1d.%02d", deviceInfo[comError].devType, deviceInfo[comError].staConfig);
      	strncpy(&sys_message[MSG_NOCOMM+1].msg[MSG_OFFSET+12],eventMsg,4);  // show device #
...
{  // for master to send lcd messages to slave
   ...
   // include inuse solid and flash bytes
   if (param.subStaAddressing) command.station=mainStation;
   else  command.station=param.activeMain;
   command.data[5]=outputvalue[devInUseSolid] & ~mainFlashAtSlave;		// was system_direction;
   command.data[6]=(outputvalue[devInUseFlash] & ~SLAVE_b) | mainFlashAtSlave;
   command.data[7]=STATION_SET;

   sendRemoteCommand(command, DEV_SENDSLAVE);
}
void deliverComControl(char *sys_state)
{  // let slave use the comm for a while
   struct RemoteMessageType mymsg;
   char waiting;
   ...
   *(int*)&mymsg.data[1]=availableDevices;  // DON'T SEEM TO USE IT
   mymsg.data[3]=remote_data[REMOTE_CIC];
   sendRemoteCommand(mymsg, DEV_SENDSLAVE);
...
{  // for slave to sync transaction count from master
   // p is the address of the unsigned long integer transaction count
   char *myp;
...
{  // for slave to handle local I/O processing
   ...
   struct RemoteMessageType message;
...
{  // Sends an INPUTS_ARE message to the host
   ...
   HBmessage.command = INPUTS_ARE;
   HBmessage.station = SLAVE;  // provide the real station number
   HBmessage.data[0] = di_carrierInChamber; // CIC
   HBmessage.data[1] = di_doorClosed; // Door closed
   HBmessage.data[2] = di_carrierArrival; // Arrival
   ...
   HBmessage.data[5] = mVERSION; // Version
   HBmessage.data[6] = mSUBVERSION; // Sub-version

   if (di_priorityRequest || (btnStatFlag==1)) HBmessage.data[7] |= 0x08;
   if (btnStatFlag==2) HBmessage.data[7] |= 0x10;
   // WHAT ABOUT SECURE AND SECURE RESET
   ...
   HBmessage.data[8] = slaveReturnStatus; // SLAVE FLAGS2
   slaveReturnStatus=0;  // I expect immediate response

   // sid = (char *)&secureCard.id;
   HBmessage.data[9] = 0; //*sid++; // secureCardID[0]
   HBmessage.data[10] = 0; //*sid++; // secureCardID[1]
   HBmessage.data[11] = 0; //*sid++; // secureCardID[2]
   HBmessage.data[12] = 0; //*sid++; // secureCardID[3]
   HBmessage.data[13] = 0; //(char) (secureCard.time/6);
   ...
   slaveSendResponse(HBmessage, payloadIPaddr); // ignore return value, will send again periodically
...
//   ip_print_ifs();
   ...
   message.station = SLAVE;  // provide the real station number
   ...
   message.data[10] = 0; //startupTimestamp.tm_year;
   message.data[11] = 0; //startupTimestamp.tm_mon;
   message.data[12] = 0; //startupTimestamp.tm_mday;
   message.data[13] = 0; //startupTimestamp.tm_hour;
   message.data[14] = 0; //startupTimestamp.tm_min;
   message.data[15] = 0; //startupTimestamp.tm_sec;
   message.data[16] = SLAVE;

   slaveSendResponse(message, lastIPaddr); // ignore return value, will send again periodically
}
char haveButtons;
void slaveProcessSubCommand(char command)
{  // commands embedded in the payload message
   //char ok;
   //char i;
...
void slaveProcessCommand(struct RemoteMessageType message)
{
   ...
   struct RemoteMessageType response;
   struct tm time;
   static char lastTransStation;
   #GLOBAL_INIT
   {  haveButtons=TRUE;
   	lastTransStation=0;
   }
   ...
   case REMOTE_PAYLOAD:  // standard data distribution message
      if (message.system == param.systemNum)  // is this for me?
      {
         arrival_from = message.data[0];
         system_state = message.data[1];
         // remoteAlert = UDPmessage.data[2];
         // transStation = UDPmessage.data[3];
         systemStation = message.data[3];
		   systemStationb=station2bit(systemStation);
		   rts_latch &= ~systemStationb; // clear latch for any active station
         mainStation = message.data[4];
         diverterStation = message.data[5];
         headDivStation = message.data[14];
         param.subStaAddressing = message.data[6];
         // transFlags = UDPmessage.data[7];
         system_direction = message.data[11];
         // purge_mode = UDPmessage.data[12];
         main2main_trans=message.data[17];

         // DO A BUNCH OF STUFF ... NEED TO FIGURE IT OUT
         ...
         {  // is transaction starting or stopping?
            ...
            {  // treat as ARE_YOU_READY signal
               ...
               // remove buttons during any transaction
               lcd_drawScreen(1,lcd_NO_BUTTONS);  // take away buttons
               haveButtons=FALSE;

            } else ready4Trans=0;  // NO TRANS = NO READY
         ...
         if (message.data[15] > 0) slaveProcessSubCommand(message.data[15]);
         else ackEvent=0;

	      // set inuse lights based on main station
   ...
   case SET_STATION_NAME:
      ...
      {  // save whole name
	      for (j=0; j<sizeof(param.station_name[message.station].name); j++) param.station_name[message.station].name[j] = message.data[j];
	   }
	   break;
   ...
   case SET_PHONE_NUMS:
      ...
      {  // save whole name
	      for (j=0; j<sizeof(param.phoneNum[message.station]); j++) param.phoneNum[message.station][j] = message.data[j];
	   }
	   break;
   ...
   case DISPLAY_MESSAGE:
      ...
      // set the inuse lights
      inUse(ON, message.data[5]);
      inUse(FLASH, message.data[6]);
      inUse(OFF, ~(message.data[5]|message.data[6]));

      // handle change of active main
   ...
   case TAKE_COMMAND:
      ...
      // take CIC data too
		remote_data[REMOTE_CIC] = message.data[3];
      ...
      //else lcd_enterSetupMode(1);
	   ...
	   slaveSendResponse(message, lastIPaddr);
	   lastComTime=SEC_TIMER;
	   break;
...
{  // view 0 for initial startup splash (no wait)
   ...
   lcd_Font("16");
   lcd_DispText("Colombo Pneumatic Tube Systems", 70, 180, MODE_NORMAL);
...
{  // Returns TRUE if names changed
   ...
   int button;
   char inKey;
   struct RemoteMessageType message;  // in order to transmit to slave
   row=0;
   changed=0;
   ...
   while ((row <= SYSTEM_NAME) && !secTimeout(menu_timeout))  // row < number of names (0..SYSTEM_NAME)
   {
      ...
      message.station=row;
      for (j=0; j<sizeof(param.station_name[row]); j++)
      {
	      message.data[j] = param.station_name[row].name[j];
      }
      // send this station name
      sendRemoteCommand(message, DEV_SENDSLAVE);
...
{  // Returns TRUE if phone numbers changed
   ...
   while ((row <= 10) && !secTimeout(menu_timeout))  // row < number of names (0..SYSTEM_NAME)
   {
      ...
      message.station=row;
      for (j=0; j<sizeof(param.phoneNum[row]); j++)
      {
	      message.data[j] = param.phoneNum[row][j];
      }
      // send this phone number
      sendRemoteCommand(message, DEV_SENDSLAVE);
   	...
   	// get entry
		if (getTransaction(entry, &log) == 0)
		{
         ...
         // wait till the socket is empty to pace the transmission
			startTime = MS_TIMER;
         while ((sock_tbused(&serverSocket) > 0) && ((MS_TIMER-startTime) < 500)) maintenance();
      }
      maintenance();
   }
...
//   char staConfig;
//};
#define DISCOVERY_DEVICES 40
#define BLANK_LINE "                                      \n"
#define DEVICES_PER_PAGE 10
int findDeviceByMAC(char * MAC)
{  // look for this device in the device structure and register it if its not there
	char i;
   char match;
   // first check if it exists
   match = 0;
   for (i=1; i<MAX_DEVADDRS; i++)
   {  if ((deviceInfo[i].active==TRUE) && (strncmp(deviceInfo[i].MAC, MAC, 6) == 0)) match = i;
   }
...
void registerDevice(struct device_discovery_type device)
{  // given the device structure, register it as belonging to this system
	char ipbuf[16];
   int idx;
   struct RemoteMessageType message;
   printf("Registering %2X%2X%2X %15s %3d %1d %3d\n",
      device.MAC[3], device.MAC[4], device.MAC[5],
      inet_ntoa(ipbuf,device.ipAddr),
      device.system, device.devType, device.staConfig);

   // capture in local structure
   // loop through deviceInfo structure until MAC match or active=false
   idx = findDeviceByMAC(device.MAC);
   deviceInfo[idx].active = TRUE;
   deviceInfo[idx].ipAddr = device.ipAddr;
	strncpy(deviceInfo[idx].MAC, device.MAC, 6);
   deviceInfo[idx].staConfig = device.staConfig;

   // send registration to device
   message.command = REGISTER_DEVICE;
   message.data[0] = device.staConfig;  // station or diverter configuration
   message.data[1] = idx;               // device number
   sendRemoteCommand(message, idx);

}
void lcd_registerDevice(struct device_discovery_type *device)
{  // show the screen allowing the entry of the device configuration and registration
   char description[25];
   unsigned long config;
   sprintf(description, "MAC %2X%2X%2X:%1d config", device->MAC[3], device->MAC[4], device->MAC[5], device->devType);
   config = device->staConfig;

	if (1==lcd_GetNumberEntry(description, &config, 1, 20, "", 2, FALSE))
   {  device->staConfig = (char)config;
   	device->system = param.systemNum;
      registerDevice(*device);
   }
}
void lcd_showDevice(struct device_discovery_type device)
{  // Show a device record on the screen
	char buf[80];
   char ipbuf[16];
   sprintf(buf, "%2X%2X%2X %15s %3d %1d.%02d %4d\n",
      device.MAC[3], device.MAC[4], device.MAC[5],
      inet_ntoa(ipbuf,device.ipAddr),
      device.system, device.devType, device.staConfig, device.delay);
   lcd_DispText(buf, 0, 0, MODE_NORMAL);
}
void lcd_showDevicePage(char page, char lastIdx, struct device_discovery_type devices[DISCOVERY_DEVICES])
{  // Show a whole page of devices give the page number and the last device index number
	// NOTE DYNAMIC C PASSES STRUCTURES BY VALUE SO THE WHOLE THING IS ON THE STACK!!!
   // PROBABLY BETTER TO PASS THE STRUCTURE BY ADDRESS
	int i;
   lcd_Cursor(8, 45); // so next lcd_DispText overwrites the above text
   for (i=(int)page*DEVICES_PER_PAGE; i<(int)(page+1)*DEVICES_PER_PAGE; i++)
      if (i<=lastIdx) lcd_showDevice(devices[i]);
      else lcd_DispText(BLANK_LINE, 0, 0, MODE_NORMAL);
}
void lcd_drawRegisterScreen()
{  // Draw the device registration screen and touch areas
	int i;
   lcd_drawScreen(8, "DEVICE REGISTRY");		// redraw main screen
	lcd_DispText("Checking Devices ...", 8, 45, MODE_NORMAL);
   lcd_Font("8x16");
   lcd_Cursor(8, 45); // so next lcd_DispText overwrites the above text
   // define the touch areas
   for (i=0; i<10; i++)
   {  // define touch areas
		lcd_DefTouchArea(i+128, TA_TYPE, TOUCH_INVERT, TA_TLXY, 10, 45+i*16, TA_SIZE, 300, 15, TA_END);
   }
}
void lcd_registerDevices()
{  // shows the devices in the network
   // Starts by requesting a network discovery and displaying devices
   char page;
   char lastPage;
   int button;
   //int devicesPerPage;
   struct device_discovery_type devices[DISCOVERY_DEVICES];
   struct RemoteMessageType UDPmessage;
   unsigned long delayTimer;
   char devIdx;
   char lastDevIdx;
   ...
   lcd_drawRegisterScreen();
   // start the discovery process
   //Payload.ackEvent = DEVICE_INFO;
	//sendPayloadToRemotes();
   UDPmessage.command = DEVICE_INFO;
   sendRemoteCommand(UDPmessage, DEV_BROADCAST);
   // wait until devices can respond
   devIdx=0;
   delayTimer = MS_TIMER;
   while ((MS_TIMER - delayTimer < 2000) && (devIdx < DISCOVERY_DEVICES))
   {	// Process messages for a while
  		tcp_tick(NULL);
      if (getRemoteMessage(&UDPmessage) > 0)
      {  // ?? processRemoteMessage(UDPmessage);  // process standard stuff
         // record discovery
         if (UDPmessage.command == DEVICE_INFO)
         {  for (i=0; i<6; i++) devices[devIdx].MAC[i] = UDPmessage.data[i];
            devices[devIdx].ipAddr = *(unsigned long*)&UDPmessage.data[6];
            devices[devIdx].system = UDPmessage.system;
            devices[devIdx].staConfig = UDPmessage.data[16];
            devices[devIdx].devType = UDPmessage.devType;
            devices[devIdx].delay = MS_TIMER-delayTimer;
            if (devIdx < DEVICES_PER_PAGE) lcd_showDevice(devices[devIdx]);
            devIdx++;
         } else
         {  //lcd_DispText(".", 0, 0, MODE_NORMAL);
            processRemoteMessage(UDPmessage);  // process standard stuff
...
// fill array with dummy records for testing
/*for (i=0; i<20; i++)
{  strcpy(devices[devIdx].MAC, "ABCDEF");
	devices[devIdx].ipAddr = 0x11223344 +i;
   devices[devIdx].system = 20;
   devices[devIdx].station = i;
   devices[devIdx].devType = 2;
   devices[devIdx].delay = MS_TIMER-delayTimer;
   devIdx++;
}*/
   // count how many pages we can show
   page=0;
   lastDevIdx = devIdx-1;
   lastPage = lastDevIdx / DEVICES_PER_PAGE;
	button=0;
   while ((button != BTN_CANCEL) && !secTimeout(menu_timeout))
   {  maintenance();
   	button = lcd_GetTouch(100);
      ...
      {  // valid button pressed
         ...
         case BTN_PREV:
         	if (page > 0) page--; else page=lastPage;  // allow wraparound
            // lcd_showTransPage(page, lastPage, format);
            lcd_showDevicePage(page, lastDevIdx, devices);
         	break;
         case BTN_NEXT:
            if (page < lastPage) page++; else page=0;  // allow wraparound
            lcd_showDevicePage(page, lastDevIdx, devices);
         	break;
         default:
            if ((button > 127) && (button < 128+DEVICES_PER_PAGE))
            {  i = (int)(page*DEVICES_PER_PAGE) + (button-128); // which item
               printf("touch item %d\n", i);
            	if (i <= lastDevIdx)
               {  lcd_registerDevice(&devices[i]);
	               // reset screen
	               lcd_drawRegisterScreen();
	               lcd_showDevicePage(page, lastDevIdx, devices);
               }
