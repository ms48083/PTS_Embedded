...
*****************************************************************/
#define FIRMWARE_VERSION "Mainstream V4.23"
#define VERS                            4
#define SUBVERS                         23
...
//   char subversion;       // Main subversion
   ...
   char reserved[99];    // to separate standard parameters from the user id's
...
//      TLmessage.data[sizeof(translog)] = param.cardL3Digits-CARDHBOFFSET; // add on 5th byte of 38 bit card id
   ...
   else if (translog.status == ESTS_CARD_SCAN)
...
{  // map secure trans data into trans log and save
   ...
   {  trans.status = ESTS_CARD_SCAN;
   	trans.trans_num = transactionCount()+1; // maybe just temporary
...
{  // Issue HTTP request to system server for status information
   ...
   if (sock_established(&tcpsock)) {
      ...
      mktm(&timeptr, param.UIDSync);
		sprintf(postContent,"sysnum=%d&trct=%ld&rno=%d&refr=%04d%02d%02d%02d%02d%02d", param.systemNum, transactionCount(), param.UIDRecNo,
      ...
      while((ii < 0) && (!Timeout(tcpWaitTimer, 2000))) { ii=sock_bytesready(&tcpsock); tcp_tick(&tcpsock); }
...
// Check for discontinuous transaction record
         ...
         {  // put up a message
            ...
            sprintf(msgbuf, " %2d USERS CHANGED   ", status);
         }
         ...
         //printf("Done %ld\n", MS_TIMER - processTime);
      } else printf("No data in sock_gets %ld\n", MS_TIMER - processTime);
...
{  // parse through input string for UID's and add or delete them as needed
	...
	{  UIndex = 6; // first user
      ...
      {  action = Ubuf[UIndex];
			...
			if (i >= 1) // at least 1 digit
         {
	         ...
	         else if (action == 'e') // sync date
	         {  if (iadd==0 && isend==0) param.UIDSync = cvtTime(UID, 0);
               // send sync to remotes
	            printf("Sync at %s\n", UID);
	         }
	         else if (action == 'l') // sync record number
	         {  if (iadd==0 && isend==0) param.UIDRecNo = atoi(UID); // only if no add or send failure
	         }
         }
   ...
   if (iadd==-1) retval=-1;  // return -1 for any add failure
   return retval;
}
int Sync_Clock(char *Buffer)
