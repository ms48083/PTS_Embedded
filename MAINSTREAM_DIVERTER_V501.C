...
#class auto
#define FIRMWARE_VERSION "DIVERTER V5.01"
#define VERSION                       5
...
// #define DCRTCP_VERBOSE
   ...
   #define MY_BASE_IP "192.168.%d.2%d"
...
{  int i;
   ...
   THIS_DEVICE = 0;  // start with no device id, main will inform
...
/********************************************************/
char transStation, diverterStation, headDivStation, transFlags;
char carrier_attn, carrier_attn2;
char system_state, systemDirection;
char ready4Trans, ackCommand, ackEvent;
...
{  // take care of business when malfunction changes state
   ...
   malfunctionActive = (system_state == MALFUNCTION_STATE) ? TRUE : FALSE;
...
{   // Handles incoming requests from the main station

	static char lastTransStation;  // to keep track of when transStation changes
	static char lastDiverterStation;
   static char lastHeadDivStation;
   static char lastSystemState;

   	#GLOBAL_INIT
      {  lastDiverterStation=0;
         lastHeadDivStation=0;
         lastTransStation=0;
      }
   ...
   /* determine what type of command */
   ackCommand = UDPmessage.command;  // Ack every command
   switch(UDPmessage.command)
   {
 	      ...
 	      if (UDPmessage.system == param.system)  // is this for me?
	      {
	        // handle completed transaction
	         ...
	         {  if (UDPmessage.data[1]==1)  // and Main says to set arrival alert
	            { // OK to set alert
	               if ((mainStation==SLAVE) && (param.subStaAddressing==TRUE))
	                     carrier_attn2 |= station2bit(UDPmessage.station);  // to flash inuse
	               else  carrier_attn |= station2bit(UDPmessage.station);  // to flash inuse
	               arrival_alert(aaSET, UDPmessage.station);
	            }
      ...
      case REMOTE_PAYLOAD:  // standard data distribution message
	      if (UDPmessage.system == param.system)  // is this for me?
	      {
	         ...
	         // handle change of diverter mapping
	         if ((UDPmessage.data[5] != lastDiverterStation)
	            || (UDPmessage.data[14] != lastHeadDivStation))
	         {  // process change of diverter
	            diverterStation = UDPmessage.data[5];
	            headDivStation = UDPmessage.data[14];
	            lastDiverterStation = UDPmessage.data[5];
	            lastHeadDivStation = UDPmessage.data[14];
	            switch (param.opMode)
	            {
	            case STD_REMOTE:
	               // stuff for standard remote
	               if (param.subStaAddressing && mainStation==SLAVE)
	                  inUse2(FLASH, station2bit(transStation));
	               else
	                  inUse(FLASH, station2bit(transStation));
	               setDiverter(lastDiverterStation);
	               break;
	            case HEAD_DIVERTER:
	               // stuff for standard head diverter
	               setDiverter(lastHeadDivStation);  // head diverter comes in different byte
	               break;
	            case HEAD_DIVERTER2:
	               // alternate head diverter
	               // main on port 1, station 1 on port 2, station 2 on port 3
	               if (lastHeadDivStation==1) // align to main on port 1
	                  setDiverter(0);  // align main
	               else
	                  setDiverter(transStation); // align to station
	               break;
	            }
	         }

	         // do we need to acknowledge ready to start transaction?
	         if (system_state==BEGIN_SEND)
	         {  // BEGIN_SEND state is for waiting for all devices to prepare and signal their readiness
	            ...
	            // could re-check diverter position but we already confirmed it
#if REMOTE_TYPE == RT_STANDARD
	            ...
	            {  // stuff for standard remote
	               ...
	               if  ( doorClosed(station2bit(transStation))
	                && ( carrierInChamber(station2bit(transStation)) || systemDirection==DIR_SEND) )
	               {  // ready
	                  ready4Trans=transStation;
	                  if (param.subStaAddressing && mainStation==SLAVE)
	                  {  inUse2(ON, station2bit(transStation));
	                  }else
	                  {  inUse(ON, station2bit(transStation));
	                  }
	               }
	               else
	               {  ready4Trans=0;                     // not ready
	                  if (param.subStaAddressing && mainStation==SLAVE)
	                        inUse2(OFF, station2bit(transStation));
	                  else  inUse(OFF, station2bit(transStation));
	                  alert(ON, station2bit(transStation));
	               }
	            }
#endif
	         }
	         ...
	         {  // is transaction starting or stopping?
	            ...
	            if (transStation==0) ready4Trans=0; // reset ready4trans
	         ...
	         if (UDPmessage.data[15] > 0) processCommand(UDPmessage.data[15]);
	         else ackEvent=0;

	         // handle blower state changes
	         if (lastSystemState != system_state)
      ...
      case SET_PARAMETERS:
	      if (UDPmessage.system == param.system)  // is this for me?
	      {
	         ...
	         //param.portMapping = UDPmessage.data[THIS_DEVICE];
...
{
#if REMOTE_TYPE == RT_DIVERTERONLY
  return 0; //(g_carrierInChamber) & stamask;
#else
...
{
#if REMOTE_TYPE == RT_DIVERTERONLY
	return 0; // g_carrierArrival & stamask;
#else
...
char requestToSend(char stamask)
{
   char indata;
#if REMOTE_TYPE == RT_DIVERTERONLY
   return 0; //(g_requestToSend & stamask);
#else
...
char requestToSend2(char stamask)
{
   char indata;
#if REMOTE_TYPE == RT_DIVERTERONLY
   return 0; //(g_requestToSend2 & stamask);
#else
   ...
   */
   ...
   if ((station>=0) && (station<9))     // Valid station #
   {
      ...
      if ((diverter_setting>0) && (diverter_setting!=di_diverterPos))
      {
         ...
         // diverterStation=station; this is only set by payload
      }
   }
   return;
}
...
int sendUDPcommand(struct UDPmessageType UDPmessage, long ipAddr)
{
   ...
   if (param.opMode == STD_REMOTE)  UDPmessage.devType = 3; // std diverter
   else                             UDPmessage.devType = 4; // head diverter
...
#endif
#if REMOTE_TYPE == RT_DIVERTERONLY
   HBmessage.station = 0; // NO DIRECT STATIONS
#endif
#if REMOTE_TYPE == RT_STANDARD
   HBmessage.station = MY_STATIONS;  // should be only one station??
#endif
...
///	   HBmessage.data[2] = latchCarrierArrival; // Arrival
   ...
   HBmessage.data[4] = rts2_latch; // RTS latch - 2nd destination
         ...
         {  // stuff for head diverter
            ...
            if (ok || di_HDArrival) HBmessage.data[REMOTE_ARRIVE]=MY_STATIONS;
            else HBmessage.data[REMOTE_ARRIVE]=0;
            HBmessage.data[REMOTE_CIC]=0;
            HBmessage.data[REMOTE_DOOR]=0; // HD has no stations  MY_STATIONS;
            HBmessage.data[REMOTE_RTS]=0;
            if (blwrError==FALSE) HBmessage.data[REMOTE_RTS2]=blowerPosition();
            else HBmessage.data[REMOTE_RTS2]=0xFF; // Blower has an error
         }
   ...
   HBmessage.data[19] = ready4Trans;
	...
	// check if diverters are ready
   // what diverter position/port is being set?
   switch (param.opMode)
   {
      case STD_REMOTE:
         i = diverterStation;
         break;
      case HEAD_DIVERTER:
         i = headDivStation;          // main, remote, or slave
         break;
      case HEAD_DIVERTER2:
         if (headDivStation==1) i=0;  // main
         else i = diverterStation;     // sub station for alternate config
         break;
   }
...
void sendDeviceInfo(void)
{  // sends the device information to the last requesting device
   ...
   message.station = MY_STATIONS;  // provide the real station number
   ...
   message.data[16] = param.portMapping;
